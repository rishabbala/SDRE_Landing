...
global xval,yval,xsig,ysig,sig1,sig2,wval,wsig,x_velocity,y_velocity,alt,now_time,last_time,center_pred_x,center_pred_y,center_x,center_y,flag_tracking,notFoundCount,flag_detected,xt_image,yt_image,width,height,P,num_detections
global flag_first_detection,flag_started_vis,flag_imageshow,detector,x_bar,y_bar,flag_apriltag,tagID,tagID_prev
flag_started_vis=0
...
def receiveimage(data):
    global x_velocity,y_velocity,roll,pitch,yaw,out,xval,yval,xsig,ysig,sig1,sig2,wval,wsig,x_velocity,y_velocity,alt,a_orig,cvFrame,flag_started_vis,a,b,c,d,flag_tracking,xt_image,yt_image,width,height,flag_detected,flag_imageshow,num_detections,x_bar,y_bar,flag_apriltag
    global tagID,tagID_prev,flag_previoustag
    bridge=CvBridge()
    a_time=rospy.get_rostime()
    #print a_time.secs
    orig_img = bridge.imgmsg_to_cv2(data,"passthrough")
    orig_img = cv2.cvtColor(orig_img, cv2.COLOR_RGB2BGR)  ####ADDED LINE
    ...
    if(alt<=1):
        ...
        Meas_noise_cov[0][0]= 20;
        Meas_noise_cov[1][1]= 20;
        Meas_noise_cov[2][2]= 1e-5;
        Meas_noise_cov[3][3]= 1e-5;
        Process_noise_cov=np.zeros((6,6))
        Process_noise_cov[0][0]= 1;
        Process_noise_cov[1][1]= 1;
        Process_noise_cov[2][2]= 2;
        Process_noise_cov[3][3]= 2;
    ...
    #barcodes = pyzbar.decode(cvFrame)
    ...
    if(alt<=1.5):
        ...
        if(num_detections>0):
            ...
            for detection in detections:
                if(detection.tag_id==tagID_prev):
                    x_bar=detection.center[0]
                    y_bar=detection.center[1]
                    tagID=detection.tag_id
                    flag_previoustag=1
                    print "tracking same tag",tagID
            ...
            if(flag_previoustag==0):
                ind_tag=random.randint(1,num_detections)
                x_bar=detections[ind_tag-1].center[0]
                y_bar=detections[ind_tag-1].center[1]
                tagID=detections[ind_tag-1].tag_id
                print "searching"
    ...
    if(num_detections>0 and info.detected==1):
        #xt_image=(center.x+x_bar)/2
        #yt_image=(center.y+y_bar)/2
        xt_image=(x_bar)
        yt_image=(y_bar)
    elif(num_detections>0 and info.detected==0):
        xt_image=x_bar
        yt_image=y_bar
...
def kalmanEst(event):
    ...
    if(flag_detected==1 or flag_apriltag==1):
        flag_tracking=1
        notFoundCount=0
        #print "in detected 1"
    elif(flag_detected==0 and flag_apriltag==0):
        if(flag_tracking==1):
    ...
    #print "center_x:",a,"center_y:",b,"width:",c,"height:",d,"detected:",flag_detected,"apriltag:",flag_apriltag,"tracking:",flag_tracking,"alt:",alt
...
def identifier():
    ...
    rospy.Subscriber('/mavros/global_position/rel_alt',Float64,Receivealt)
    dt=0.2
    F=np.zeros((6,6))
    H=np.zeros((4,6))
    F[0][0]=1;F[0][2]=dt;F[1][1]=1;F[1][3]=dt;F[2][2]=1;F[3][3]=1;F[4][4]=1;F[5][5]=1
